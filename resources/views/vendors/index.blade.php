    ...
    <div class="container mx-auto px-6 py-8">
        ...
        <div class="flex items-center justify-between mb-5">
            ...
            <div class="flex items-center space-x-2">
                <h1 class="text-5xl font-extrabold text-main-dark  italic mb-1">Vendors</h1>
                <!-- Search Icon -->
                <div class="px-2 relative">
                    ...
                    <!-- Search Bar -->
                    <form id="search-bar" action="{{ route('vendors.index') }}" method="GET"
        ...
        <div class="mt-8 overflow-hidden rounded-lg shadow-lg">
            <table class="min-w-full bg-white shadow-md rounded-lg overflow-hidden bg-opacity-70">
                <!-- Table Header -->
                ...
                <tr>
                    @foreach ([
                        ...
                        <th class="px-6 py-3 text-left">
                            <a href="{{ route('vendors.index', array_merge(request()->all(), ['sort' => $field, 'direction' => ($sortField === $field && $sortDirection === 'asc') ? 'desc' : 'asc'])) }}"
                ...
                @forelse ($vendors as $vendor)
                    @php
                        $trashedClass = $vendor->trashed() ? 'text-gray-400 bg-gray-100 bg-opacity-0' : '';
                    @endphp
                    <tr class="hover:bg-accent-superlight transition {{ $trashedClass }}">
                        <td class="px-6 py-4 {{ $trashedClass }}">{{ $vendor->id }}</td>
                        <td class="px-6 py-4 {{ $trashedClass }}">{{ $vendor->name }}</td>
                        <td class="px-6 py-4 {{ $trashedClass }}">{{ $vendor->email }}</td>
                        <td class="px-6 py-4 {{ $trashedClass }}">{{ $vendor->phone }}</td>
                        <td class="px-6 py-4 {{ $trashedClass }}">{{ $vendor->address }}</td>
                        <td class="px-6 py-4 flex space-x-4 justify-center items-center {{ $trashedClass }}">
                            ...
                            <form action="{{ $vendor->trashed() ? route('vendors.restore', $vendor->id) : route('vendors.destroy', $vendor) }}" method="POST" class="inline">
                                @csrf
                                @if ($vendor->trashed())
                                    <button type="submit"
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-6 py-4 text-center text-main-dark">
                            No vendors found.
                        </td>
                    </tr>
                @endforelse
