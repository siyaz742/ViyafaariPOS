    ...
    <div class="container mx-auto px-6 py-8">
        ...
        <div class="flex items-center justify-between mb-5">
            <div class="flex items-center space-x-2">
                <h1 class="text-4xl font-extrabold text-main-dark italic mb-1">Edit Product</h1>
            </div>
        ...
        <div class="mt-8 py-6 rounded-lg">
            <form action="{{ route('products.update', $product) }}" method="POST">
                ...
                @php
                    $fields = [
                        ['name' => 'name', 'label' => 'Product Name', 'type' => 'text', 'value' => old('name', $product->name), 'required' => true],
                        ['name' => 'description', 'label' => 'Description', 'type' => 'textarea', 'value' => old('description', $product->description), 'required' => true],
                        ['name' => 'price', 'label' => 'Price', 'type' => 'number', 'value' => old('price', $product->price), 'step' => '0.01', 'required' => true],
                        ['name' => 'stock_quantity', 'label' => 'Stock Quantity', 'type' => 'text', 'value' => old('stock_quantity', $product->stock_quantity), 'required' => true, 'readonly' => true],
                    ];
                @endphp
                    ...
                    <div class="mb-6">
                        ...
                        @if($field['type'] == 'textarea')
                            <textarea
                                ...
                                maxlength="120">{{ $field['value'] }}</textarea>
                        @else
                            <input
                                type="{{ $field['type'] }}"
                                id="{{ $field['name'] }}"
                ...
                <!-- Vendor Select -->
                <div class="mb-6">
                    <label for="vendor_id" class="block bg-gradient-to-l from-main-light75 to-main-dark75 text-white pl-3 py-1 rounded-t-lg font-semibold">
                        Vendor
                    </label>
                    <select
                        name="vendor_id"
